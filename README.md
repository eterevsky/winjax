...
- `toolchains/` — Bazel override repositories and the compiler wrapper:
  `local_config_cc/` (CC toolchain + `winjax_cc_wrapper.py`),
  `local_config_cuda_win/`, `cuda_repos/<name>/` (one tiny repo per CUDA
  component). Machine-specific pieces (`winjax_cuda.bazelrc`, rendered
  toolchain files, junction dirs) are generated by `configure.py` from
  `toolchains/templates/` and ignored.
- `patches/` — Windows-port patch series for Bazel external repositories
  (triton, rmm, raft, rapids_logger, abseil, protobuf, eigen, jax,
  local_config_rocm, cccl). Applied by `python configure.py
  --patch-externals`; see `patches/README.md`.
