...
def if_cuda_hermetic_clang(if_true, if_false = []):
    ...
    Hermetic C++ toolchain is not used on Windows; always returns an empty list.
    """
    return []
...
def cuda_default_copts():
    ...
    return if_cuda([
        "-x", "cuda",
        "-DGOOGLE_CUDA=1",
    ] + ["--cuda-gpu-arch=sm_120"]) + if_cuda_clang_opt(
...
def if_cuda_newer_than(wanted_ver, if_true, if_false = []):
    ...
    configured_version = "13.3".rsplit("_", 1)[-1]
    ...
    if True and (wanted_major, wanted_minor) <= (configured_major, configured_minor):
      return if_true
    return if_false
