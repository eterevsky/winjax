...
def if_gpu_is_configured(if_true, if_false = []):
    ...
    # winjax: this repository was generated without ROCm, but CUDA is enabled
    # via --@local_config_cuda//:enable_cuda, so key on that config setting
    # instead of unconditionally returning the "no GPU" branch.
    return select({
...
def if_cuda_or_rocm(if_true, if_false = []):
    ...
    # winjax: see if_gpu_is_configured above.
    return select({
        "@local_config_cuda//:is_cuda_enabled": if_true,
        "//conditions:default": if_false,
    })
...
def get_rbe_amdgpu_pool(is_single_gpu = False):
    return "linux_x64_gpu" if is_single_gpu else "linux_x64_multigpu"
